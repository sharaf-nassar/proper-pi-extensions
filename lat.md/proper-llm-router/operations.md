...
There is no build step. Pi loads the TypeScript source directly, while Node uses experimental type stripping for tests. `package.json` and `package-lock.json` pin TypeScript, Node declarations, and development copies of the Pi coding-agent and TUI host packages for strict no-emit diagnostics, plus `@earendil-works/pi-server`, which Pi 0.85.0's package root imports without declaring; see the dependency note in `../proper-base/operations.md`. Both Pi packages remain peer dependencies at runtime so the host supplies one compatible instance. Package `prepack`, unit tests, type checking, and the standalone smoke are offline.
