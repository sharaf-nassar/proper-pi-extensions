...
- `llm-router.ts` contains configuration, model selection, judge calls, quota probes, swaps, pi event handlers, notices, and configuration UI.
- `exemplars.jsonl` contains measured verifier outcomes used as optional few-shot evidence.
- `smoke.ts` contains deterministic logic checks followed by one live judge and CPA route.
- `package.json` names the local pi package, registers `llm-router.ts`, and exposes the smoke command; `package-lock.json` pins diagnostic-only Node and pi types.
