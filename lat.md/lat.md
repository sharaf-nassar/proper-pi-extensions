...
- [[proper-model-prompts]]: repository index entry
...
All six configured packages must trust the `publish-npm.yml` workflow in `sharaf-nassar/proper-pi-extensions` with environment `npm-release`. A brand-new package receives one maintainer-authenticated initial publish because npm cannot configure trust before the package exists, and its trusted publisher must then be registered before the workflow can publish it; `proper-pacify` has its initial publish but not yet that trust registration. `proper-compact` and `proper-model-prompts` are not yet published and additionally require their first publish plus protected environment/tag-policy setup before release. The GitHub environment has no required reviewer or wait timer, and its custom deployment policies currently allow the four published package tag prefixes; adding `proper-compact-v*` and `proper-model-prompts-v*` is a maintainer prerequisite, not a development side effect. Repository tag rules supply the maintainer-only tag boundary; the release maintainer also needs ruleset bypass for the atomic `main` version-commit push. npm token publishing is disabled after the trusted path is proven.
...
Biome enforces formatting, recommended lint rules, JSON syntax and duplicate-key checks, explicit-any rejection in runtime source, and focused or skipped test rejection. The five TypeScript packages enable strict diagnostics, unchecked-index checks, exact optional properties, unused checks, and no-emit compilation. Markdown, shell, YAML, TOML, spelling, file hygiene, package-lock consistency, package contents, and lat.md links are also checked.
