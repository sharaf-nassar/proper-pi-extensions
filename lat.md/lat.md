...
`bump auto` takes no package argument: it iterates every configured package, mechanically skips packages with no commits since their last tag, and for each changed package makes one AI pass that decides the semver level and writes the release notes together. The pass receives the commit messages plus the actual diff, size-capped with lock files excluded, and is instructed to treat the diff as the source of truth so changes that commits omit or under-describe still count. It prompts for confirmation only when a major bump is decided — the prompt accepts the release, declines that package, or overrides the bump level to major, minor, or patch, and manual `bump` confirmations offer the same overrides — and otherwise releases sequentially through the normal publish path, so an interrupted run can simply be rerun. Outside npm mode the same flow releases the repository once.
