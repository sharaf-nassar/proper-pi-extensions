...
A command fixture verifies `/clear` replaces the session before restoring the outgoing model, all seven thinking levels, both session Fast states, and the session context-window choice.

Separate outgoing and replacement extension instances prove that no stale API is used and no messages or session metadata are copied into `ctx.newSession()`. The replacement command selects the exact provider/model pair before restoring thinking, even when model selection resets it to a different default. A second capture verifies the real in-memory Fast flag, and repeated restoration cannot flip it. Malformed references, invalid thinking or Fast values, missing models, and failed model activation leave settings alone and report errors. Cancelled switches leave the outgoing session intact, and an absent outgoing model keeps native `/new` behavior. A session `max` choice travels in the encoded settings and is appended to the replacement session, while an invalid carried value is rejected.

## Sticky defaults fixture

A Pi 0.87.1 host fixture creates a real `AgentSession`, `SettingsManager`, `ModelRuntime`, resource loader, and extension lifecycle around the compatibility adapter.
...
A session integration fixture applies the real Pi 0.87.1 `KeybindingsManager` to the installed editor factory.
...
A fourth fixture counts how often pi's native footer render runs behind the decoration. It proves repeated renders at one width reuse the memoized lines and produce identical output without scanning the session again, that a new width still renders because the cached lines cannot be reused across widths, that moving the session leaf id recomputes the totals, and that a new context window on the same model recomputes them too. The first fixture's stub derives its leaf id from the context percentage, mirroring the real invariant that context usage cannot move without an appended entry, compaction, or branch change.
...
Context-window fixtures verify `/tokens` parsing, mode precedence, per-backend maxima, notices, and a real `AgentSession`.

Argument cases accept `max` or `default` with an optional trailing `global` and reject everything else. Completion cases list the scopes after a completed mode with the session scope first, and an editor fixture proves Tab on a mode appends the space and opens the scope menu, a typed space after a mode opens it, Tab on the session scope completes without reopening, and other commands, texts, and an already open menu are left alone. A fake session and registry prove no choice keeps the registry model; a session `max` installs a 922,000-token copy without mutating the registry object; global `max` applies when the session defers through a `null` entry, a session `default` restores the registry object, and a simulated registry refresh is corrected on the next apply. The write guard windows each assigned eligible model immediately, passes ineligible and `default` models through untouched, survives a stale remover after a reload installs a newer guard, and restores a plain writable property on removal. Backend cases prove CLIProxyAPI, `openai-codex`, and `openai` models (including API GPT-5.5 and GPT-5.4) reach 922,000, while Codex GPT-5.5, non-OpenAI models, other providers, unlisted OpenAI models, and a `models.json` override already at the maximum stay untouched. Notices report the trigger as window minus reserve, warn when the current context is past it, and warn when auto-compaction is off and the context exceeds the window. A Pi host fixture resolves the live session through the sticky-defaults capture for Pi's native `openai-codex/gpt-6-astra`, applies `max` under the guard, proves a provider re-registration through the extension API (which makes Pi write the fresh registry object with no event) keeps 922,000 on the refreshed model, and proves `getContextUsage()` reports 922,000 while a `compaction.modelOverrides` reserve still resolves for the copy.
...
Host-boundary fixtures instantiate Pi 0.87.1's real session, settings, model, selector, manager, extension-runner, and editor surfaces.
