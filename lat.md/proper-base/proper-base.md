...
16. Ctrl+V and Ctrl+Shift+V invoke Pi's clipboard paste action; image paths become `[image N]` markers without moving the cursor away from the replacement, marker deletion leaves the cursor at the removed marker, source paths stay in a text-only overlay, terminal capabilities remain unchanged, and intact markers expand before history storage and Pi submission.
