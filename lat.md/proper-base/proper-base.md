...
- `src/autocomplete-details.ts` owns overlay lifecycle, boxed rendering, terminal positioning, selected-description updates, descending `/model` argument ordering, argument re-trigger after an accepted command completion, and immediate submission of selected model completions.
- `src/default-model.ts` writes the selected provider and model ID into Pi's `settings.json` startup defaults, honoring the `stickyModel` opt-out.
...
- `src/rail-setting.ts` persists the session action rail's on/off choice in the agent directory and splices its toggle into Pi's native settings selector through a guarded container wrapper.
...
36. A `set` or `cycle` model selection is written to Pi's `defaultProvider`/`defaultModel` startup keys; a `restore` selection, the `llm-router` routing placeholder, an unchanged pair, a missing or damaged `settings.json`, and `"stickyModel": false` all write nothing, and no other setting is modified.
