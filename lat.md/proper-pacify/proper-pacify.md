...
What the rewrite changed renders on the user message itself, not inside the entry. A display-only markdown transformer rewrites settled user-message markdown at render time: it matches the rendered text against each recorded entry's child user message in the session and, when the recorded original differs, displays a word-level LCS diff in the prompt block — deletions struck through in the theme's removed-diff color, insertions in its added-diff color, kept text left as ordinary markdown. Entries flagged as cancelled or as command dispatches never register for pairing: neither is followed by its own rewrite, so their next user-message child is a later unrelated prompt, and pairing it would strike out text the user never typed. A skill command is the one command that is not flagged. Pi expands `/skill:name args` after the rewrite into a skill block followed by the rewritten argument, stores that whole text as the user message, and renders the block as its own collapsible component with only the argument as the user prompt. The transformer therefore receives the argument alone, so the lookup parses the stored message with Pi's `parseSkillBlock` and pairs that argument with the body of the typed command, minus its slash token. A prompt template stays flagged: its stored message is the substituted template, which never equals the rewrite. Raw ANSI survives Pi's markdown renderer and is reopened across wrapped lines, so the styling needs no markdown syntax. No second entry, schema change, or dependency is involved, and a resumed session re-derives the same diff from the stored pair.
...
Cancellation and failure are reported through `ctx.ui.notify()` so they appear in the transcript beside the entry the call already wrote; a failure that falls open to the original prompt adds no second entry. A cancelled rewrite — and a `/pacify` invocation that ends without sending — appends a `cancelled` marker entry instead: it renders `pacify cancelled` with the discarded text on expand, and because it becomes the leaf, the next unpacified user message is its child rather than the pending entry's, keeping the pairing rule above safe. A rewritten command input other than a skill command is flagged at logging time for the same reason: its dispatch appends no user message at all. The extension sets no footer status, because such a message in the footer competes with other extensions for one truncated line.
